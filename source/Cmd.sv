...
localparam st_read_con = 6;             //Receives Data From Senser State
...
//Write Process Master I2C
    ...
    always@ (posedge clk) begin
        if (!rstn) begin
            ...
            if (s_cmd_ready == 1) begin
                if (state == st_write || state == st_read || state == st_read_con) begin
                    rs_cmd_valid <= 1;
                end else begin
                    rs_cmd_valid <= 0;
                end 
...
//===============================================================================================
    ...
    always@ (posedge clk) begin
        if (!rstn) begin
            ...
            if (state == st_read_con || state == st_Int) begin         //Tx not busy && in Read mode
